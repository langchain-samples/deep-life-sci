**Concept: a Deep Agents implementation with the skills and tooling to act as a research assistant for life scientists**
...
The API has enough sharp edges that the tools have to absorb them—silent query mangling, malformed PMIDs returning the wrong paper, hard batch caps that fail as HTTP 200. Each guard lives next to the call it protects in `research_agent/sources/`; the details don't belong in this doc.
...
Subagents via LangChain’s dynamic subagents (https://docs.langchain.com/oss/python/deepagents/dynamic-subagents), dispatched from interpreter code with `task()`. Give them a `response_schema` so the synthesis step gets structured objects instead of prose to parse. Cheap model on the leaves (Haiku for abstract Q&A, Sonnet at the root) is worth showing—it’s the cost story for anyone running thousands of abstracts.

Both the interpreter and dynamic subagents are beta APIs, and may move between deepagents releases.

Web search beyond PubMed came later, and is itself a PTC tool (`sources/web.py`) rather than a search spec bound to the root model—binding it would land every retrieved page in root context, which is the one thing this design spends its effort avoiding.